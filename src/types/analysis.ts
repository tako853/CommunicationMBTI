...
// コミュニケーションタイプの4軸
export interface CommunicationAxes {
  // 軸1: 伝える力 - Assert(A) vs Reserved(R)
  assertiveness: 'A' | 'R';
  // 軸2: 聞く力 - Connect(C) vs Distill(D)
  listening: 'C' | 'D';
  // 軸3: 非言語を伝える力 - Faceful(F) vs Subtle(S)
  nonverbalExpression: 'F' | 'S';
  // 軸4: 非言語を読み取る力 - Perceptive(P) vs Tell-me(T)
  nonverbalReading: 'P' | 'T';
}

// 各軸のスコア（0-100）
export interface CommunicationAxisScores {
  assertiveness: number; // 高い = Assert, 低い = Reserved
  listening: number; // 高い = Connect, 低い = Distill
  nonverbalExpression: number; // 高い = Faceful, 低い = Subtle
  nonverbalReading: number; // 高い = Perceptive, 低い = Tell-me
}
